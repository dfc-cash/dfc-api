...
// to sender server in order to obtain dfc.toml file and signing key.
func (r *AuthRequest) VerifySignature(sender string) error {
	...
	senderStellarToml, err := dfctoml.GetStellarTomlByAddress(sender)
	if err != nil {
		return errors.Wrap(err, "Cannot get dfc.toml of sender domain")
	}

	if senderStellarToml.SigningKey == "" {
		return errors.New("No SIGNING_KEY in dfc.toml of sender")
	}
