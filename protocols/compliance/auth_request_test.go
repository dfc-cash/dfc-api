...
func TestValidateError(t *testing.T) {
	...
	authData := &AuthData{
		Sender:         "bartekdfc.org",
		NeedInfo:       false,
		Tx:             "&^%",
		AttachmentJSON: "abc",
	}

	assert.EqualError(t, authData.Validate(), "sender: bartekdfc.org does not validate as dfc_address;tx: &^% does not validate as base64;attachment: abc does not validate as json")
}
