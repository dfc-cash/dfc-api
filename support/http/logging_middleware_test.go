...
func TestHTTPMiddleware(t *testing.T) {
	...
	if assert.Len(t, logged, 4, "unexpected log line count") {
		assert.Equal(t, "starting request", logged[0].Mdfcge)
		assert.Equal(t, "starting request", logged[2].Mdfcge)
		assert.Equal(t, "finished request", logged[1].Mdfcge)
		assert.Equal(t, "finished request", logged[3].Mdfcge)
	}
