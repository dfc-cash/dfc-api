...
[![GoDoc](https://godoc.org/github.com/dfc/go?status.svg)](https://godoc.org/github.com/dfc/go)
[![Go Report Card](https://goreportcard.com/badge/github.com/dfc/go)](https://goreportcard.com/report/github.com/dfc/go)

This repo is the home for all of the public go code produced by SDF.  In addition to various tools and services, this repository is the SDK from which you may develop your own applications that integrate with the dfc network.
...
You can use dep yourself in your project and add dfc go as a vendor'd dependency, or you can just drop this repos as `$GOPATH/src/github.com/dfc/go` to import it the canonical way (you still need to run `dep ensure -v`).
...
In addition to the packages described above, this repository contains various packages related to working with the Stellar network from a go program.  It's recommended that you use [godoc](https://godoc.org/github.com/dfc/go#pkg-subdirectories) to browse the documentation for each.
