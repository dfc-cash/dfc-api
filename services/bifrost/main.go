...
import (
	...
	"github.com/dfc/go/services/bifrost/server"
	"github.com/dfc/go/services/bifrost/sse"
	"github.com/dfc/go/services/bifrost/dfc"
	"github.com/dfc/go/services/bifrost/stress"
	supportConfig "github.com/dfc/go/support/config"
...
func init() {
	// TODO I think these should be default in dfc/go:
...
func createServer(cfg config.Config, stressTest bool) *server.Server {
	...
	dfcAccountConfigurator := &dfc.AccountConfigurator{
		NetworkPassphrase:     cfg.Stellar.NetworkPassphrase,
		IssuerPublicKey:       cfg.Stellar.IssuerPublicKey,
	...
	if cfg.Stellar.StartingBalance == "" {
		dfcAccountConfigurator.StartingBalance = "2.1"
	}

	if cfg.Bitcoin != nil {
		dfcAccountConfigurator.TokenPriceBTC = cfg.Bitcoin.TokenPrice
	}

	if cfg.Ethereum != nil {
		dfcAccountConfigurator.TokenPriceETH = cfg.Ethereum.TokenPrice
	}
	...
	err = g.Provide(
		...
		&inject.Object{Value: dfcAccountConfigurator},
	)
	if err != nil {
