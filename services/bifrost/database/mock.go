...
func (m *MockDatabase) CreateAddressAssociation(chain Chain, dfcAddress, address string, addressIndex uint32) error {
	a := m.Called(chain, dfcAddress, address, addressIndex)
	return a.Error(0)
}
...
func (m *MockDatabase) GetAssociationByStellarPublicKey(dfcPublicKey string) (*AddressAssociation, error) {
	a := m.Called(dfcPublicKey)
	return a.Get(0).(*AddressAssociation), a.Error(1)
}
