...
CREATE TABLE address_association (
  ...
  dfc_public_key varchar(56) NOT NULL UNIQUE,
  created_at timestamp NOT NULL,
  PRIMARY KEY (chain, address_index, address, dfc_public_key),
...
CREATE TABLE transactions_queue (
  ...
  dfc_public_key varchar(56) NOT NULL,
  ...
  CONSTRAINT valid_dfc_public_key CHECK (char_length(dfc_public_key) = 56)
);
