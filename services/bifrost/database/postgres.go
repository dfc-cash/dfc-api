...
type transactionsQueueRow struct {
	...
	StellarPublicKey string          `db:"dfc_public_key"`
}
...
func (d *PostgresDatabase) CreateAddressAssociation(chain Chain, dfcAddress, address string, addressIndex uint32) error {
...
func (d *PostgresDatabase) GetAssociationByStellarPublicKey(dfcPublicKey string) (*AddressAssociation, error) {
	addressAssociationTable := d.getTable(addressAssociationTableName, nil)
	row := &AddressAssociation{}
	where := map[string]interface{}{"dfc_public_key": dfcPublicKey}
	err := addressAssociationTable.Get(row, where).Exec()
	if err != nil {
		switch errors.Cause(err) {
		case sql.ErrNoRows:
