...
func (s *Server) handlerGenerateAddress(w stdhttp.ResponseWriter, r *stdhttp.Request, chain database.Chain) {
	...
	dfcPublicKey := r.PostFormValue("dfc_public_key")
	_, err := keypair.Parse(dfcPublicKey)
	if err != nil || (err == nil && dfcPublicKey[0] != 'G') {
		log.WithField("dfcPublicKey", dfcPublicKey).Warn("Invalid dfcPublicKey")
		w.WriteHeader(stdhttp.StatusBadRequest)
		return
	}
	...
	err = s.Database.CreateAddressAssociation(chain, dfcPublicKey, address, index)
	if err != nil {
		log.WithFields(log.F{
			"err":              err,
			"chain":            chain,
			"index":            index,
			"dfcPublicKey": dfcPublicKey,
			"address":          address,
