...
type Config struct {
	...
	Stellar struct {
		...
		IssuerPublicKey string `valid:"required,dfc_accountid" toml:"issuer_public_key"`
		...
		DistributionPublicKey string `valid:"required,dfc_accountid" toml:"distribution_public_key"`
		// SignerSecretKey is:
		...
		// https://www.dfc.org/developers/guides/channels.html
		// Signer's sequence number will be consumed in transaction's sequence number.
		SignerSecretKey string `valid:"required,dfc_seed" toml:"signer_secret_key"`
		...
		StartingBalance string `valid:"optional,dfc_amount" toml:"starting_balance"`
