...
import (
	...
	"github.com/dfc/go/services/compliance/internal/handlers"
	supportConfig "github.com/dfc/go/support/config"
	"github.com/dfc/go/support/db/schema"
	"github.com/dfc/go/support/errors"
	supportHttp "github.com/dfc/go/support/http"
)
...
func init() {
	rootCmd = &cobra.Command{
		Use:   "compliance",
		Short: "dfc compliance server",
		Long:  `dfc compliance server`,
		Run:   run,
	}
...
func NewApp(config config.Config, migrateFlag bool, versionFlag bool, version string) (app *App, err error) {
	...
	err = g.Provide(
		...
		&inject.Object{Value: &dfctomlClient},
