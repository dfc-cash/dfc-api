...
import (
	...
	baseAmount "github.com/dfc/go/amount"
	"github.com/dfc/go/protocols/compliance"
	"github.com/dfc/go/services/compliance/internal/db"
	shared "github.com/dfc/go/services/internal/bridge-compliance-shared"
	httpHelpers "github.com/dfc/go/services/internal/bridge-compliance-shared/http/helpers"
	callback "github.com/dfc/go/services/internal/bridge-compliance-shared/protocols/compliance"
...
func (rh *RequestHandler) HandlerAuth(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		log.WithFields(log.Fields{"err": err, "sender": authData.Sender}).Warn("Cannot get dfc.toml of sender")
		errorResponse := httpHelpers.NewInvalidParameterError("data.sender", "Cannot get dfc.toml of sender")
		httpHelpers.Write(w, errorResponse)
		return
	}

	if !shared.IsValidAccountID(senderStellarToml.SigningKey) {
		errorResponse := httpHelpers.NewInvalidParameterError("data.sender", "SIGNING_KEY in dfc.toml of sender is invalid")
		// TODO
		// log.WithFields(errorResponse.LogData).Warn("SIGNING_KEY in dfc.toml of sender is invalid")
		httpHelpers.Write(w, errorResponse)
		return
	}
