...
func (rh *RequestHandler) HandlerSend(w http.ResponseWriter, r *http.Request) {
	...
	if authDataEntity != nil {
		var dfcToml *dfctoml.Response
		dfcToml, err = rh.StellarTomlResolver.GetStellarToml(authDataEntity.Domain)
		if err != nil {
		...
		if dfcToml.AuthServer == "" {
			log.Print("No AUTH_SERVER in dfc.toml")
			helpers.Write(w, callback.AuthServerNotDefined)
			return
		}

		rh.sendAuthData(w, dfcToml.AuthServer, []byte(authDataEntity.AuthData))
		return
	}
	...
	dfcToml, err := rh.StellarTomlResolver.GetStellarToml(domain)
	if err != nil {
		log.WithFields(log.Fields{
			"destination": request.Destination,
			"err":         err,
	...
	rh.sendAuthData(w, dfcToml.AuthServer, data)
}
