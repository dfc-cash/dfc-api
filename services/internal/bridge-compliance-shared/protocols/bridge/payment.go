...
import (
	...
	"github.com/dfc/go/services/internal/bridge-compliance-shared/protocols"
	complianceServer "github.com/dfc/go/services/internal/bridge-compliance-shared/protocols/compliance"
...
var (
	...
	PaymentCannotResolveDestination = &helpers.ErrorResponse{Code: "cannot_resolve_destination", Mdfcge: "Cannot resolve federated Stellar address.", Status: http.StatusBadRequest}
	// PaymentCannotUseMemo is an error response
	PaymentCannotUseMemo = &helpers.ErrorResponse{Code: "cannot_use_memo", Mdfcge: "Memo given in request but federation returned memo fields.", Status: http.StatusBadRequest}
	// PaymentSourceNotExist is an error response
	PaymentSourceNotExist = &helpers.ErrorResponse{Code: "source_not_exist", Mdfcge: "Source account does not exist.", Status: http.StatusBadRequest}
	// PaymentAssetCodeNotAllowed is an error response
	PaymentAssetCodeNotAllowed = &helpers.ErrorResponse{Code: "asset_code_not_allowed", Mdfcge: "Given asset_code not allowed.", Status: http.StatusBadRequest}

	// compliance

	// PaymentPending is an error response
	PaymentPending = &helpers.ErrorResponse{Code: "pending", Mdfcge: "Transaction pending. Repeat your request after given time.", Status: http.StatusAccepted}
	// PaymentDenied is an error response
	PaymentDenied = &helpers.ErrorResponse{Code: "denied", Mdfcge: "Transaction denied by destination.", Status: http.StatusForbidden}
)
...
type PaymentRequest struct {
	...
	Source string `form:"source" valid:"optional,dfc_seed"`
	// Sender address (like alice*dfc.org)
	Sender string `form:"sender" valid:"optional,dfc_address"`
	// Destination address (like bob*dfc.org)
	Destination string `form:"destination" valid:"optional,dfc_destination"`
	// ForwardDestination
	...
	Amount string `form:"amount" valid:"required,dfc_amount"`
	// Code of the asset destination should receive
	AssetCode string `form:"asset_code" valid:"optional,dfc_asset_code"`
	// Issuer of the asset destination should receive
	AssetIssuer string `form:"asset_issuer" valid:"optional,dfc_accountid"`
	// Only for path_payment
	SendMax string `form:"send_max" valid:"optional,dfc_amount"`
	// Only for path_payment
	SendAssetCode string `form:"send_asset_code" valid:"optional,dfc_asset_code"`
	// Only for path_payment
	SendAssetIssuer string `form:"send_asset_issuer" valid:"optional,dfc_accountid"`
