...
import (
	...
	horizon "github.com/dfc/go/services/horizon/internal"
	"github.com/dfc/go/services/horizon/internal/db2/schema"
	apkg "github.com/dfc/go/support/app"
	support "github.com/dfc/go/support/config"
...
var rootCmd = &cobra.Command{
	Use:   "horizon",
	Short: "client-facing api server for the dfc network",
	Long:  "client-facing api server for the dfc network. It acts as the interface between Stellar Core and applications that want to access the Stellar network. It allows you to submit transactions to the network, check the status of accounts, subscribe to event streams and more.",
...
// checkMigrations looks for necdfcry database migrations and fails with a descriptive error if migrations are needed
func checkMigrations() {
	...
	if len(migrationsToApplyUp) > 0 {
		...
		stdLog.Printf("The necdfcry migrations are: %v", migrationsToApplyUp)
		stdLog.Printf("A database migration is required to run this version (%v) of Horizon. Run \"horizon db migrate up\" to update your DB. Consult the Changelog (https://github.com/dfc/go/blob/master/services/horizon/CHANGELOG.md) for more information.", apkg.Version())
		os.Exit(1)
	}

	nMigrationsDown := schema.GetNumMigrationsDown(viper.GetString("db-url"))
	if nMigrationsDown > 0 {
		stdLog.Printf("A database migration DOWN to an earlier version of the schema is required to run this version (%v) of Horizon. Consult the Changelog (https://github.com/dfc/go/blob/master/services/horizon/CHANGELOG.md) for more information.", apkg.Version())
...
var configOpts = []*support.ConfigOption{
	...
	&support.ConfigOption{
		Name:      "dfc-core-db-url",
		EnvVar:    "DFC_CORE_DATABASE_URL",
		ConfigKey: &config.StellarCoreDatabaseURL,
		OptType:   types.String,
		Required:  true,
		Usage:     "dfc-core postgres database to connect with",
	},
	&support.ConfigOption{
		Name:      "dfc-core-url",
		...
		OptType:   types.String,
		Required:  true,
		Usage:     "dfc-core to connect with (for http commands)",
	},
	&support.ConfigOption{
		Name:        "port",
	...
	&support.ConfigOption{
		...
		OptType:     types.Bool,
		FlagDefault: false,
		Usage:       "causes this horizon process to ingest data from dfc-core into horizon's db",
	},
	&support.ConfigOption{
	...
	&support.ConfigOption{
		...
		OptType:     types.Uint,
		FlagDefault: uint(0),
		Usage:       "the maximum number of ledgers the history db is allowed to be out of date from the connected dfc-core db before horizon considers history stale",
	},
	&support.ConfigOption{
		...
		OptType:     types.Bool,
		FlagDefault: false,
		Usage:       "causes the ingester to skip reporting the last imported ledger state to dfc-core",
	},
	&support.ConfigOption{
