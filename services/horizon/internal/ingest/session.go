...
import (
	...
	"github.com/dfc/go/meta"
	"github.com/dfc/go/services/horizon/internal/db2/history"
	"github.com/dfc/go/services/horizon/internal/ingest/participants"
	"github.com/dfc/go/support/errors"
	ilog "github.com/dfc/go/support/log"
	sTime "github.com/dfc/go/support/time"
...
func (is *Session) ingestEffects() {
	...
	case xdr.OperationTypeCreatePassiveOffer:
		...
		// KNOWN ISSUE:  dfc-core creates results for CreatePassiveOffer operations
...
func (is *Session) ingestTrades() {
	...
	for i, trade := range trades {
		// dfc-core will opportunisticly garbage collect invalid offers (in the
...
// reportCursorState makes an http request to the configured dfc-core server
// to report that it has finished processing the data being ingested.  This
// allows dfc-core to free that storage when next it runs its own
// maintenance.
func (is *Session) reportCursorState() error {
	...
	core := &dfccore.Client{URL: is.StellarCoreURL}
