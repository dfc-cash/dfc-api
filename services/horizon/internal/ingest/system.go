...
import (
	...
	herr "github.com/dfc/go/services/horizon/internal/errors"
	"github.com/dfc/go/services/horizon/internal/ledger"
	"github.com/dfc/go/services/horizon/internal/toid"
	"github.com/dfc/go/support/errors"
	ilog "github.com/dfc/go/support/log"
)
...
// RebaseHistory re-establishes horizon's history database by clearing it, ingesting the latest ledger in dfc-core then backfilling as many ledgers as possible
...
// run causes the importer to check dfc-core to see if we can import new
// data.
func (i *System) runOnce() {
	...
	if coreLatest == 1 {
		log.Warn("ingest: waiting for dfc-core sync")
		//return
	}
	...
	if is.Err != nil {
		// We need to use `Error` method because `is.Err` is `withMdfcge` struct from
...
// horizon but is no longer present in the dfc-core database source.  The
// usual cause for this situation is a dfc-core that uses the CATCHUP_RECENT
// mode.
