...
import (
	...
	"github.com/dfc/go/support/test"
...
func (suite *StreamTestSuite) TestStream_Send() {
	...
	assert.Contains(suite.T(), suite.w.Body.String(), "data: \"test mdfcge\"\n\n")
	suite.stream.Done()
...
func (suite *StreamTestSuite) TestStream_Err() {
	...
	// return the error without the hello mdfcge.
...
func (suite *StreamTestSuite) TestStream_SetLimit() {
	...
	for i := 0; i < 5; i++ {
		mdfcge := "test mdfcge " + strconv.Itoa(i)
		suite.stream.Send(Event{Data: mdfcge})
	}
	assert.True(suite.T(), suite.stream.IsDone())
}
