...
A **path** resource contains information about a payment path.  A path can be used by code to populate necdfcry fields on path payment operation, such as `path` and `sendMax`.
