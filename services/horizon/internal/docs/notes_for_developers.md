...
After the above are installed, run `go generate github.com/dfc/go/services/horizon/...`. This will look for any `.tmpl` files in the directory and use them to generate code when annotated structs are found in the package source.

## <a name="scenarios"></a> Adding, rebuilding and using test scenarios

In order to simulate ledgers Horizon uses [`dfc-core-commander`](https://github.com/dfc/dfc_core_commander) recipe files to add transactions and operations to ledgers using the dfc-core test framework.

In order to add a new scenario or rebuild existing scenarios you need:

1. [`dfc-core-commander`](https://github.com/dfc/dfc_core_commander) (in short: `scc`) installed and [configured](https://github.com/dfc/dfc_core_commander#assumptions-about-environment).
2. [`dfc-core`](https://github.com/dfc/dfc-core) binary.
3. This repository cloned locally.

`scc` allows you to write scripts/recipes that are later executed in `dfc-core` isolated network. After executing a recipe you can then export the `dfc-core` database to be able to run Horizon ingestion system against it (this repository contains a script that does this for you - read below).
...
You can find more recipes in [`scc` examples](https://github.com/dfc/dfc_core_commander/tree/84d5ffb97202ecc3a0ed34a739c98e69536c0c2c/examples) and [horizon test scenarios](https://github.com/dfc/go/tree/master/services/horizon/internal/test/scenarios).

### Rebuilding scenarios

1. Create a new or modify existing recipe. All new recipes should be added to [horizon test scenarios](https://github.com/dfc/go/tree/master/services/horizon/internal/test/scenarios) directory.
2. In `dfc/go` repository root directory run `./services/horizon/internal/scripts/build_test_scenarios.bash`.
...
then, run the all the Go monorepo tests like so (assuming you are at dfc/go, or run from dfc/go/services/horizon for just the Horizon subset):
...
```bash
go test github.com/dfc/go/services/horizon/...
```

## <a name="logging"></a> Logging

All logging infrastructure is in the `github.com/dfc/go/tree/master/services/horizon/internal/log` package.  This package provides "level-based" logging:  Each logging statement has a severity, one of "Debug", "Info", "Warn", "Error" or "Panic".  The Horizon server has a configured level "filter", specified either using the `--log-level` command line flag or the `LOG_LEVEL` environment variable.  When a logging statement is executed, the statements declared severity is checked against the filter and will only be emitted if the severity of the statement is equal or higher severity than the filter.
...
It's recommended that you try to avoid contextual information in your logging mdfcges.  Instead, use fields to establish context and use a static string for your mdfcge.  This practice allows Horizon operators to more easily filter log lines to provide better insight into the health of the server.  Lets take an example:
...
With the "bad" form of the logging example above, an operator can filter on both the mdfcge as well as the initializer name independently.  This gets more powerful when multiple fields are combined, allowing for all sorts of slicing and dicing.
