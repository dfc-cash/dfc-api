...
import (
	...
	horizonContext "github.com/dfc/go/services/horizon/internal/context"
	"github.com/dfc/go/services/horizon/internal/db2/core"
	"github.com/dfc/go/services/horizon/internal/db2/history"
	"github.com/dfc/go/services/horizon/internal/ingest"
	"github.com/dfc/go/services/horizon/internal/ledger"
	"github.com/dfc/go/services/horizon/internal/logmetrics"
	"github.com/dfc/go/services/horizon/internal/operationfeestats"
	"github.com/dfc/go/services/horizon/internal/paths"
	"github.com/dfc/go/services/horizon/internal/reap"
	"github.com/dfc/go/services/horizon/internal/simplepath"
	"github.com/dfc/go/services/horizon/internal/txsub"
	"github.com/dfc/go/support/app"
	"github.com/dfc/go/support/db"
...
// CoreSession returns a new session that loads data from the dfc core
// database. The returned session is bound to `ctx`.
...
func (a *App) UpdateStellarCoreInfo() {
	...
	if err != nil {
		log.Warnf("could not load dfc-core info: %s", err)
		return
	}
	...
	if resp.Info.Network != a.config.NetworkPassphrase {
		log.Errorf(
			"Network passphrase of dfc-core (%s) does not match Horizon configuration (%s). Exiting...",
			resp.Info.Network,
...
func (a *App) Tick() {
	...
	// update ledger state, operation fee state, and dfc-core info in parallel
	wg.Add(3)
