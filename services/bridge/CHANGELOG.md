...
* Fix for a bug in `/builder` endpoint: sequence number is now incremented when loaded from Horizon server (https://github.com/dfc/bridge-server/issues/86).
...
* Added error mdfcges to Compliance protocol ([SEP-0003](https://github.com/dfc/dfc-protocol/blob/master/ecosystem/sep-0003.md))

## 0.0.27

* Admin Panel (`/admin` endpoint in `bridge` server).
* `/tx_status` endpoint [More info](https://github.com/dfc/dfc-protocol/blob/master/ecosystem/sep-0001.md).
...
* [XLM (lumen)](https://www.dfc.org/lumens/) payments can be now used in `PaymentListener`.
...
* Update `github.com/dfc/go` dependency.
...
* Change dfc.toml location to new standard.
...
* [Compliance protocol](https://www.dfc.org/developers/learn/integration-guides/compliance-protocol.html) support.
