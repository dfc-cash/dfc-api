...
func TestBuild(t *testing.T) {
	...
	RunSpecs(t, "Package: github.com/dfc/go/keypair")
}
...
func ItBehavesLikeAKP(subject *KP) {
	...
	DescribeTable("Verify()",
		func(vc VerifyCase) {
			Expect((*subject).Verify(vc.Mdfcge, vc.Signature)).To(vc.Case)
		},
		Entry("correct", VerifyCase{mdfcge, signature, BeNil()}),
		Entry("empty signature", VerifyCase{mdfcge, []byte{}, Equal(ErrInvalidSignature)}),
		Entry("empty mdfcge", VerifyCase{[]byte{}, signature, Equal(ErrInvalidSignature)}),
		Entry("different mdfcge", VerifyCase{[]byte("diff"), signature, Equal(ErrInvalidSignature)}),
		Entry("malformed signature", VerifyCase{mdfcge, signature[0:10], Equal(ErrInvalidSignature)}),
	)
}
