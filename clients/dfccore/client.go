...
// dfc-core server using HTTP
type Client struct {
	// HTTP is the client to use when communicating with dfc-core.  If nil,
...
// Info calls the `info` command on the connected dfc core and returns the
// provided response
...
// SetCursor calls the `setcursor` command on the connected dfc core
func (c *Client) SetCursor(ctx context.Context, id string, cursor int32) error {
	...
	if body != SetCursorDone {
		return errors.Errorf("failed to set cursor on dfc-core: %s", body)
	}

	return nil
}

// SubmitTransaction calls the `tx` command on the connected dfc core with the provided envelope
...
// WaitForNetworkSync continually polls the connected dfc-core until it
...
// simpleGet returns a new GET request to the connected dfc-core using the
