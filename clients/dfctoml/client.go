...
func (c *Client) GetStellarToml(domain string) (resp *Response, err error) {
	...
	// https://github.com/dfc/go/pull/24#discussion_r89909696
	if err != nil && limitReader.(*io.LimitedReader).N == 0 {
		err = errors.Errorf("dfc.toml response exceeds %d bytes limit", StellarTomlMaxSize)
		return
	}
...
// url returns the appropriate url to load for resolving domain's dfc.toml
// file
