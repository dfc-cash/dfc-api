...
import (
	...
	proto "github.com/dfc/go/protocols/federation"
	"github.com/dfc/go/support/errors"
)

// LookupByAddress performs a federated lookup following to the dfc
...
// "name" type is a legacy holdover from the legacy dfc network's federation
...
// LookupByAccountID performs a federated lookup following to the dfc
...
// ForwardRequest performs a federated lookup following to the dfc
...
func (c *Client) getFederationServer(domain string) (string, error) {
	...
	if err != nil {
		return "", errors.Wrap(err, "get dfc.toml failed")
	}

	if stoml.FederationServer == "" {
		return "", errors.New("dfc.toml is missing federation server info")
	}
