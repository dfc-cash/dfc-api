...
This is a small tool, written in Go, for working with `dfc-core` history archives directly.
It is a standalone tool that does not require `dfc-core`, or any other programs.

It is much smaller and simpler than `dfc-core`, and is intended only for archive-maintenance tasks.
...
```
$ go install github.com/dfc/go/tools/dfc-archivist
```

## Usage

```
inspect dfc history archive

Usage:
  dfc-archivist [flags]
  dfc-archivist [command]
...
Use "dfc-archivist [command] --help" for more information about a command.
```

## Specifying history archives

Unlike `dfc-core`, `dfc-archivist` does not run subprocesses to access history archives;
...
archives much more quickly. This is necdfcry to handle bulk operations on archives with many
thousands of files efficiently.

### S3 backend

`dfc-archivist` supports reading from and writing to any S3-compatible storage.
...
```
$ dfc-archivist status --s3endpoint ams3.digitaloceanspaces.com s3://bucketname/prefix
```
...
```
$ dfc-archivist status http://s3-eu-west-1.amazonaws.com/history.dfc.org/prd/core-testnet/core_testnet_001

       Archive: http://s3-eu-west-1.amazonaws.com/history.dfc.org/prd/core-testnet/core_testnet_001
...
```
$ dfc-archivist mirror http://s3-eu-west-1.amazonaws.com/history.dfc.org/prd/core-testnet/core_testnet_001 file://local-archive

2016/02/10 18:27:09 mirroring http://s3-eu-west-1.amazonaws.com/history.dfc.org/prd/core-testnet/core_testnet_001 -> file://local-archive
...
```
$ dfc-archivist --last 1024 mirror http://s3-eu-west-1.amazonaws.com/history.dfc.org/prd/core-testnet/core_testnet_001 file://local-archive

2016/02/10 19:14:01 mirroring http://s3-eu-west-1.amazonaws.com/history.dfc.org/prd/core-testnet/core_testnet_001 -> file://local-archive
...
```
$ dfc-archivist scan file://local-archive
...
```
$ dfc-archivist --last 4096 scan file://local-archive
...
$ dfc-archivist --low 1000000 --high 1006000 scan file://local-archive
...
```
$ dfc-archivist --verify --last 4096 scan file://local-archive
...
$ dfc-archivist repair file://local-archive file://broken-archive
...
```
 dfc-archivist dumpxdr local-archive/transactions//00/20/de/transactions-0020de7f.xdr.gz
